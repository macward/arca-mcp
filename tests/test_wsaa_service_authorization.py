...
@pytest.mark.asyncio
async def test_service_authorized(cert_key_pair, mocker):
    cert_path, key_path = cert_key_pair
    mocker.patch("arca_mcp.wsaa.login.call_login_cms", return_value=SUCCESS_RESPONSE)

    result = await validate_service_authorization(cert_path, key_path, "wsfe")
    assert result.ok is True
    assert result.token is not None


@pytest.mark.asyncio
async def test_service_unauthorized_alias_fault(cert_key_pair, mocker):
    cert_path, key_path = cert_key_pair
    mocker.patch(
...
@pytest.mark.asyncio
async def test_empty_service_rejected(cert_key_pair):
    cert_path, key_path = cert_key_pair

    result = await validate_service_authorization(cert_path, key_path, "")
    assert result.ok is False
    assert result.cause == ArcaErrorCause.SERVICE_UNAUTHORIZED
...
@pytest.mark.asyncio
async def test_whitespace_service_rejected(cert_key_pair):
    cert_path, key_path = cert_key_pair
    result = await validate_service_authorization(cert_path, key_path, "   ")
    assert result.ok is False
    assert result.cause == ArcaErrorCause.SERVICE_UNAUTHORIZED
...
@pytest.mark.asyncio
async def test_arca_service_enum_can_be_used(cert_key_pair, mocker):
    cert_path, key_path = cert_key_pair
    mocker.patch("arca_mcp.wsaa.login.call_login_cms", return_value=SUCCESS_RESPONSE)

    result = await validate_service_authorization(cert_path, key_path, ArcaService.WS_SR_PADRON_A4)
    assert result.ok is True


@pytest.mark.asyncio
async def test_arbitrary_service_string_accepted(cert_key_pair, mocker):
    """No debe haber whitelist rígida — cualquier string no vacío es válido a priori,
    la decisión la toma WSAA."""
    cert_path, key_path = cert_key_pair
    mocker.patch("arca_mcp.wsaa.login.call_login_cms", return_value=SUCCESS_RESPONSE)

    result = await validate_service_authorization(cert_path, key_path, "ws_servicio_nuevo_2027")
    assert result.ok is True
